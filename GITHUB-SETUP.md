# Publish version 0.3.1 on GitHub
...
3. In the repository, choose **Add file → Upload files**. Upload the contents of that inner folder: `module.json`, `README.md`, `package.json`, `scripts`, `styles`, `data`, `docs`, and `tests`. The repository root should contain `module.json`, not another enclosing folder. Commit the upload.
4. Open **Releases → Draft a new release**.
5. Create tag **v0.3.1** against the main branch. Title the release **Heroic Character Creator v0.3.1 — Prototype**.
...
```text
https://github.com/jeremyrobertdavison/heroic-character-creator/releases/download/v0.3.1/module.json
https://github.com/jeremyrobertdavison/heroic-character-creator/releases/download/v0.3.1/heroic-character-creator.zip
```
...
For future versions, increment `version` and the tagged `download` URL in both manifest copies, create the matching Git tag, and attach both files. Keep the stable `manifest` URL for Foundry updates. Do not mark a manifest as verified for a Foundry generation until it has been checked there; 0.3.1 targets generation 13 but still needs the live-world checklist.

Suggested release notes:

> Core catalogue expansion: 321 powers, 57 traits, 48 tags, 30 origins and 18 occupations, with prerequisite checks, package grants and specialization choices. Existing selected automation remains available; newly added effects require manual resolution. See docs/CORE-CATALOGUE.md. Live-world testing is still required.

If the repository already exists, upload the updated source, create a new **v0.3.1** release, and attach the new ZIP and manifest. Existing users can then use Foundry’s module update check. Keep the manifest asset named exactly `module.json`.
