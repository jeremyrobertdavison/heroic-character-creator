# Publish version 0.2.0 on GitHub
...
5. Create tag **v0.2.0** against the main branch. Title the release **Heroic Character Creator v0.2.0 — Prototype**.
...
```text
https://github.com/jeremyrobertdavison/heroic-character-creator/releases/download/v0.2.0/module.json
https://github.com/jeremyrobertdavison/heroic-character-creator/releases/download/v0.2.0/heroic-character-creator.zip
```
...
For future versions, increment `version` and the tagged `download` URL in both manifest copies, create the matching Git tag, and attach both files. Keep the stable `manifest` URL for Foundry updates. Do not mark a manifest as verified for a Foundry generation until it has been checked there; 0.2.0 targets generation 13 but still needs the live-world checklist.

Suggested release notes:

> Included rules references and Heroic Actions for the existing 11 powers, three traits and two tags. Adds GM-assisted Focus costs, stances, reactions, special damage, timed benefits and recovery-backed content updates. Keeps hard creation limits. See docs/HEROIC-ACTIONS.md for coverage and remaining manual decisions. Live-world testing is still required.

If the repository already exists, upload the updated source, create a new **v0.2.0** release, and attach the new ZIP and manifest. Existing users can then use Foundry’s module update check. Keep the manifest asset named exactly `module.json`.
